...
const App = () => {
  ...
  const handleFetchFormats = async () => {
    ...
    try {
      const response = await fetch(
        `https://youtube-video-downloader-backend-steel.vercel.app/formats?url=${encodeURIComponent(
          videoUrl
        )}`
      );
      ...
      setFormats(data.formats); // Set available formats
    } catch (error) {
      console.error("Error fetching formats:", error);
      alert("Error fetching formats");
    }
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(
        `https://youtube-video-downloader-backend-steel.vercel.app/download?url=${encodeURIComponent(
          videoUrl
        )}&itag=${selectedFormat}`
      );
      ...
      const a = document.createElement("a");
      a.href = url;
      a.setAttribute("download", "video.mp4"); // Customize this based on format
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
